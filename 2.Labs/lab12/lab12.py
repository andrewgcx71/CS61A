...
def prune_min(t):
    ...
    if t.is_leaf() or len(t.branches)==1:
        ...
        if t.branches[0].label > t.branches[1].label:
            t.branches.remove() #remove() will remove the fisrt element in the list, it doesn't take any passing arguments
...
def remainders_generator(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    def inner_generator_func(m,r):#m-1 is the number of natural numbers, r is the remainder
        if r==0:
            mutilplier=1
        else:
            mutilplier=0
        for _ in range(m-1):
            yield m*mutilplier+r
            mutilplier+=1

    i=0
    for _ in range(m):
        yield inner_generator_func(m,i)
        i+=1
...
def mapl(lst, fn):
    ...
    """
    "*** YOUR CODE HERE ***"
    if not lst.rest: # if 1st.rest is empty
