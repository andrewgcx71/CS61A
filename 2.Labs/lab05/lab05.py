...
def distance(city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    return sqrt((get_lat(city1)-get_lat(city2))**2+(get_lon(city1)-get_lon(city2))**2)*1.0
...
def closer_city(lat, lon, city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    anoymous_cty=make_city("anoymous", lat,lon)
    d_frm_cty1=distance(anoymous_cty,city1)
    d_frm_cty2=distance(anoymous_cty,city2)
    if d_frm_cty1>d_frm_cty2:
...
def nut_finder(t):
    ...
    if is_leaf(t):
        return False
    found=False
    for node in [nut_finder(branch) for branch in branches(t)]:
        if node==True:
            found=True
    return found
