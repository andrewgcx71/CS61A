...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 9
            assert (isinstance(place.ant,ContainerAnt) and place.ant.can_contain(self) or (isinstance(self,ContainerAnt) and self.can_contain(place.ant))), 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        temp_head=self.place
        position=0


        while temp_head and (not temp_head is beehive):
            if temp_head.bees and (self.min_range<=position and position <=self.max_range):
                return random_or_none(temp_head.bees)
            temp_head=temp_head.entrance
            position+=1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        copyofbees=list(self.place.bees)
        if self.armor <= amount:
            for bee in copyofbees:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting==0:
            if self.place.bees:
                bee_about_to_be_eaten=random_or_none(self.place.bees)
                self.eat_bee(bee_about_to_be_eaten)
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 11
        "*** YOUR CODE HERE ***"
        Place.add_insect(self,insect) #inherit all the functionalities from orginal add_insect method from base class
        if insect.is_watersafe==False:#add additioanl functionalities
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.counter!=0: #an imposter
            self.reduce_armor(self.armor)
        else: # True QueenAnt
            ThrowerAnt.action(self,gamestate)
            head=self.place.exit
            while head:
                if head.ant:
                    if isinstance(head.ant,ContainerAnt):
                        if head.ant.contained_ant:
                            if head.ant.contained_ant.buffed==False:
                                head.ant.contained_ant.damage=head.ant.contained_ant.damage*2
                                head.ant.contained_ant.buffed=True

                    if head.ant.buffed==False:
                        head.ant.damage=head.ant.damage*2
                        head.ant.buffed=True
                head=head.exit
        # END Problem 13
