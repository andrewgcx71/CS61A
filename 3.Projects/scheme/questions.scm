...
(define (zip pairs)
    (cond 
          ( (null? pairs)  '(() ()))
          ( else
    (define first (map (lambda (n) (car n) ) pairs))
    (define second (map (lambda (n) (car (cdr n)) ) pairs))
    (list first second)
         )
    )
    )
...
(define define? (check-special 'deine))
  ...
  (cond ((atom? expr)
        ...
        ((or (lambda? expr)
         ...
         (let ((form   (car expr))
               ...
               (body   (cddr expr)))
           ; BEGIN PROBLEM 18
           (define rest (map let-to-lambda body))
           (append (list form params) rest)/                                             
           ; END PROBLEM 18
           ))
        ((let? expr) 
         (let ((values (cadr expr))
           ...
           ; BEGIN PROBLEM 18
            ...
            (define args (map let-to-lambda (car (cdr (zip values)))))
